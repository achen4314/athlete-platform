...
def process_single_video(video_id: int, app) -> bool:
    """
    处理单个视频

    Args:
        video_id: Video 表的 id
        app: Flask 应用实例
    ...
    with app.app_context():
        ...
        logger.info("开始处理视频: id=%d, file=%s", video_id, video.filename)
        ...
        if not filepath:
            video.status = 'error'
            video.analysis_results = json.dumps({'error': '视频文件不存在'})
            db.session.commit()
        ...
        try:
            ...
            result = analyzer.analyze_video(filepath, video.test_type)
            ...
            logger.info("视频处理完成: id=%d, type=%s", video_id, result.get('test_type', 'unknown'))
            return True
...
def process_pending_videos(app):
    """扫描并处理所有待处理视频"""
    with app.app_context():
        ...
        for video in pending:
            process_single_video(video.id, app)
